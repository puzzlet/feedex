...
class DiffFormatter(EntryFormatter):
    ...
    def format_entries(self, entries):
        ...
        if self.last_message:
            generator = format_diff(self.last_message, message)
        else:
    ...
    def format_diff_message(self, message):
        if not self.last_message:
            self.last_message = message
            return message
        return '\n'.join(format_diff(self.last_message, message))

def format_diff(str1, str2):
    ...
    for tag, i1, i2, j1, j2 in diff.get_opcodes():
        ...
        if tag == 'replace':
            for sub_a, sub_b in itertools.zip_longest(a[i1:i2], b[j1:j2],
                    fillvalue=''):
                yield ''.join(format_diff_line(sub_a, sub_b))

def format_diff_line(str1, str2):
